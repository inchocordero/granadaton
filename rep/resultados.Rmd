...
#Calcula índice gini
gini <- ddply(.data=resultados,.(Año,Mes,Tipo), summarise, RESULT=Gini(Votos))
gini <- gini[1:(nrow(gini)-1),]
gini.x.eleccion <- data.frame(Fecha=paste(gini$Año,gini$Mes,sep="/"),Tipo=gini$Tipo,gini=gini$RESULT)

#Calcula entropía
entropy <- ddply(.data=resultados,.(Año,Mes,Tipo), summarise, RESULT=entropy(Votos))
...
```{r, echo=FALSE, message=FALSE,results='hide',warning=FALSE}
ggplot(gini.x.eleccion,aes(x=Fecha,y=gini,color=Tipo))+geom_point(stat='identity')+theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
